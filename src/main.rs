...
pub(crate) const GLOBAL_SERVICE_HOME_V2: &'static str = "/var/db/backone";

#[cfg(target_os = "linux")]
pub(crate) const GLOBAL_SERVICE_HOME_V2: &'static str = "/var/lib/backone";
...
pub(crate) const GLOBAL_SERVICE_HOME_V1: &'static str = "/var/db/backone";

#[cfg(target_os = "linux")]
pub(crate) const GLOBAL_SERVICE_HOME_V1: &'static str = "/var/lib/backone";

#[cfg(target_os = "macos")]
pub(crate) const MAC_BUNDLE_ID: &'static str = "com.backone.BackOne-UI";
    ...
    unsafe {
        START_ON_LOGIN = out.map_or(false, |app_list| {
            String::from_utf8(app_list.stdout.to_ascii_lowercase())
                .map_or(false, |app_list| app_list.contains("backone"))
        });
    }
}
...
fn refresh_windows_start_on_login() {
    ...
    let enabled = startup.map_or(false, |startup| {
        startup.get_value::<String, &str>("BackOneUI").is_ok()
    });
    unsafe {
...
fn tray_icon_name() -> String {
    let icon_path = std::env::temp_dir().join("backone-tray-icon.ico");
    ...
    */
        ...
        if client.lock().is_online() {
            ...
            menu.push(TrayMenuItem::Text {
                ...
                handler: Some(Box::new(move || {
                    ...
                    notify("Copied this node's BackOne address to clipboard.", None);
                })),
            });
                    ...
                    handler: Some(Box::new(move || {
                        for _ in 0..2 {
                            ...
                            if previous_start_on_login {
                                ...
                                let _ = out.map(|app_list| {
                                    String::from_utf8(app_list.stdout).map(|app_list| {
                                        app_list.split('\n').for_each(|app| {
                                            if app.to_ascii_lowercase().contains("backone") {
                            ...
                            if previous_start_on_login != unsafe { START_ON_LOGIN } {
                                ...
                                let _ = Command::new("/usr/bin/tccutil").arg("reset").arg("AppleEvents").arg("com.backone.BackOne-UI").output();
                            }
                        }
                    ...
                    handler: Some(Box::new(move || {
                        ...
                        if startup.is_ok() {
                            ...
                            if unsafe { START_ON_LOGIN } {
                                let _ = startup.delete_value("BackOneUI");
                            } else {
                                let exe = String::from(
                                    std::env::current_exe().unwrap().to_str().unwrap(),
                                );
                                let _ = startup.set_value("BackOneUI", &exe);
                            }
                        }
                        refresh_windows_start_on_login();
                        dirty_flag2.store(true, std::sync::atomic::Ordering::Relaxed);
            ...
            menu.push(TrayMenuItem::Text {
                text: "Waiting for BackOne system service...".into(),
                checked: false,
                disabled: true,
                handler: None,
            });
    ...
    loop {
        ...
        for (nwid, auth_url, status) in auth_required_networks.iter() {
            ...
            {
                notify(
                    format!("BackOne network {} requires SSO authentication. Select 'Open SSL Login URL' to proceed.", nwid).as_str(),
...
fn main() {
        ...
        #[cfg(not(windows))]
        {
            unsafe {
                let _ = std::env::var("HOME").map_or_else(
                    |_| {
                        APPLICATION_HOME = String::from("/tmp/backone_ui");
                    },
                    |home_dir| {
                        APPLICATION_HOME = format!("{}/.backone_ui", home_dir);
                    },
                );
            }
        }
    ...
    #[cfg(target_os = "macos")]
    {
        if !Path::new(unsafe { NETWORK_CACHE_PATH.as_str() }).is_file() {
            ...
            let _ = plist::Value::from_file(format!("{}/networkinfo.dat", unsafe {
                &APPLICATION_HOME
            }))
            .map(|old_plist| {
