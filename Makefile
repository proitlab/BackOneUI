...
mac: libui
	...
	lipo -create target/aarch64-apple-darwin/$(CARGO_TARGET_DIR)/backone_desktop_ui target/x86_64-apple-darwin/$(CARGO_TARGET_DIR)/backone_desktop_ui -output target/$(CARGO_TARGET_DIR)/backone_desktop_ui
	make mac-assemble-app

mac-assemble-app: FORCE
	rm -rf BackOne.app
	cp -av mac-app-template/BackOne.app .
	mkdir -p BackOne.app/Contents/MacOS
	cp -f target/$(CARGO_TARGET_DIR)/backone_desktop_ui BackOne.app/Contents/MacOS/BackOne
	xattr -cr BackOne.app
	$(CODESIGN) -f --options=runtime -s $(CODESIGN_APP_CERT) BackOne.app
...
ubuntudeb_64:	linux
	fpm -t deb \
	-p backone-ui.0.1.0-1.amd64.deb \
	--version 0.1.0-1 \
	--architecture amd64
